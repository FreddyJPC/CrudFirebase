...
addDeck(deck: Deck) {
  ...
  getDecks() {
    return collectionData (this.deckCollection, {idField: 'id'}) as Observable<Deck[]>;
  }

  getDeck(id: string) {
     return from (getDoc( doc(this.firestore, 'decks', id))).pipe(map (spanshot => spanshot.data() as Deck)
     );
  }
  ...
  deleteDeck(id: string) {
    const deckDoc = doc(this.firestore, 'decks', id);
    deleteDoc(deckDoc);
}
}
